...
import { submitTextToImage, submitImageEdit } from "@/lib/kie-client";
  ...
  try {
    ...
    if (num_images > 1) {
      return NextResponse.json(
        { 
          success: false, 
          error: "Kie.ai only supports generating 1 image per request" 
        },
        { status: 400 }
      );
    }
    ...
    const webhookUrl =
      process.env.NODE_ENV === "development"
        ? "https://similarly-just-caiman.ngrok-free.app/api/nano-banana/kie/webhook"
        : "https://nanobanana.org/api/nano-banana/kie/webhook";
    ...
    try {
      ...
      console.error("Kie API error:", kieError);
      return NextResponse.json(
        {
          success: false,
          error: "Failed to submit to Kie.ai image generation service",
    ...
    try {
      ...
      // 如果处理失败，记录错误但返回任务ID（任务已提交到 Kie.ai）
    ...
    console.error("Kie Submit API error:", error);
    return NextResponse.json(
      {
        success: false,
