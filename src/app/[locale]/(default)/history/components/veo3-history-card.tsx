...
interface Veo3HistoryCardProps {
  task: Veo3Task;
  onViewVideo: () => void;
  onDelete: (taskId: string) => void;
}

export default function Veo3HistoryCard({
  task,
  onViewVideo,
  onDelete,
}: Veo3HistoryCardProps) {
  ...
  return (
    <Card className="group relative overflow-hidden transition-all duration-300 hover:scale-[1.02] hover:shadow-xl border-green-500/10 hover:border-green-500/30">
      ...
      <div className="relative p-4 space-y-4">
        ...
        {/* 操作按钮 */}
        <CardActions task={task} onViewVideo={onViewVideo} onDelete={onDelete} />
      </div>
    </Card>
  );
}
...
function CardVideoPreview({
  ...
  if (task.status === "completed" && task.video_720p_url) {
    return (
      <div
      ...
      >
        ...
        {/* HD badge */}
        {task.has_1080p && (
          <div className="absolute top-2 right-2 bg-green-600/90 text-white text-xs px-2 py-1 rounded font-semibold">
            HD
          </div>
        )}
      </div>
...
// 信息栏子组件
function CardInfo({ task }: { task: Veo3Task }) {
  const t = useTranslations("history");
  ...
  return (
    <div className="flex items-center justify-between text-xs text-muted-foreground">
      ...
      <div className="flex items-center gap-2">
        {task.input.has_image && (
          <span className="flex items-center gap-1 text-green-600">
            <ImageIcon className="h-3 w-3" />
            I2V
          </span>
        )}
        <span className="text-green-600 font-medium">{task.model}</span>
      </div>
    </div>
  );
}
...
function CardActions({
  ...
  return (
    <div className="flex gap-2">
      {task.status === "completed" && task.video_720p_url && (
        <Button
          size="sm"
          variant="outline"
