...
interface Sora2HistoryCardProps {
  task: Sora2Task;
  onViewVideo: () => void;
  onDelete: (taskId: string) => void;
}

export default function Sora2HistoryCard({
  task,
  onViewVideo,
  onDelete,
}: Sora2HistoryCardProps) {
    ...
    return (
      ...
      >
        ...
        <div className="absolute top-2 right-2 bg-green-600/90 text-white text-xs px-2 py-1 rounded font-semibold uppercase">
          {task.input.model || "SORA 2"}
        </div>
        {/* I2V badge */}
        {task.input.imageUrls && task.input.imageUrls.length > 0 && (
...
// 信息栏子组件
function CardInfo({ task }: { task: Sora2Task }) {
  const t = useTranslations("history");
